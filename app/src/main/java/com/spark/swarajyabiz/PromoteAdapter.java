...
import io.reactivex.rxjava3.annotations.NonNull;
    ...
    public class PromoteAdapter extends RecyclerView.Adapter<PromoteAdapter.ShopViewHolder> {
        ...
        private Set<Integer> selectedItems = new HashSet<>(); // Keep track of selected items
        private OnItemSelectionChangedListener itemSelectionChangedListener;
        private SparseBooleanArray itemSelectedState = new SparseBooleanArray(); // To track selection state
        private Set<Integer> promotedShopPositions = new HashSet<>();// Store positions of promoted shops
        ...
        @NonNull
        @Override
        public PromoteAdapter.ShopViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.promote_shop_list, parent, false);
            return new PromoteAdapter.ShopViewHolder(itemView);
        }

        @Override
        public void onBindViewHolder(@NonNull PromoteAdapter.ShopViewHolder holder, int position) {
            ...
            holder.bind(shop, position);
             ...
             shopcontactNumber = shop.getContactNumber();

            System.out.println("feghfg  " +shop.getContactNumber());
...
//                            PromoteShop clickedShop = filteredShopList.get(position);
            ...
            // Set the checkbox state based on the itemSelectedState
//            holder.checkBox.setChecked(itemSelectedState.get(position, true));
        }
        ...
        public void toggleSelection(int position) {
            boolean isPromoted = promotedShopPositions.contains(position);

            if (isPromoted) {
                promotedShopPositions.remove(position);
                selectedItems.remove(position); // Uncheck the shop
            } else {
                promotedShopPositions.add(position);
                selectedItems.add(position); // Check the shop
            }

            for (int selectedItem : selectedItems) {
                itemSelectedState.put(selectedItem, true);
            }
...
//        }
        ...
        public PromoteAdapter( Context context, List<PromoteShop> shopList, Set<Integer> promotedShopPositions, SharedPreferences sharedPreferences) {
            this.shopList = shopList;
            this.context = context;
            this.filteredShopList = new ArrayList<>(shopList);
            this.promotedShopPositions = promotedShopPositions;
...
//                        Log.d("TAG", "Removing from hePromoteYou: " + promoteShopsRef.toString());
//
                        ...
                        promotedShopsRef.removeValue()
                                .addOnCompleteListener(task -> {
                                    if (task.isSuccessful()) {
                                        ...
                                        promoteShopsRef.child(contactNumber).addListenerForSingleValueEvent(new ValueEventListener() {
                                            @Override
                                            public void onDataChange(@androidx.annotation.NonNull DataSnapshot snapshot) {
                                                if (snapshot.exists()){
                                                    String key = snapshot.getKey();
                                                    System.out.println("sdvdsv " +key);
                                                }
